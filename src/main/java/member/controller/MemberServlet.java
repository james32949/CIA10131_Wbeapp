...
public class MemberServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		if ("update".equals(action)) { // 來自update_mem_input.jsp
			...
			MemberService memSvc = new MemberService();
			MemberVO memVOUP = memSvc.updateMember(member_name, member_password, member_email, member_phone,
					member_address, member_state, member_gender, member_birthday, member_id);
		...
		if ("Upimage".equals(action)) { // 來自update_mem_input.jsp 的上傳圖片
			...
			Part part = req.getPart("member_img");
...
//				part.write(file.toString());
			...
			MemberService memSvc = new MemberService();
			MemberVO memVO = memSvc.upImage(image, member_id);
		
			/***********************************修改完成,準備轉交**********************************************************************/	

			memVO= memSvc.getOneMember(member_id);
			req.setAttribute("memVO", memVO); //
			String url = "/member/update_mem_input.jsp";
			RequestDispatcher successView = req.getRequestDispatcher(url); 
		...
		if ("insert".equals(action)) { // 來自addMember
			...
			/*************************** 2.開始增新 ***************************************/
			MemberService memSvc = new MemberService();
			memVO = memSvc.addMember(member_name, member_account, member_password, member_email, member_phone, "測試用資料",
					0, 0, null, null);
